 ...
 * @author James O'Brien (modified for Drupal jQuery namespacing)
 *
 * @param elem
 * @constructor
 */
...
CardJs.prototype.initCardNumberInput = function () {
  ...
  //@JOB Browser autofills
  this.cardNumberInput.on('blur', function () {
...
CardJs.prototype.initNameInput = function () {
  ...
  if (!CardJs.elementHasAttribute(this.nameInput, "name")) {
    this.nameInput.attr("name", "card-name");
  }
  ...
  //@JOB
  this.nameInput.attr("x-autocompletetype", "card-name");
  this.nameInput.attr("autocompletetype", "cc-name");
  this.nameInput.attr("autocorrect", "off");
  this.nameInput.attr("spellcheck", "off");
  this.nameInput.attr("autocapitalize", "on");
  //SagePay
  //this.nameInput.attr("data-card-details", "cardholder-name");
...
CardJs.prototype.setupExpiryInput = function () {
  ...
  //
    ...
    this.expiryMonthYearInput.blur(function () {
      //@JOB The following code will read for credit card browser autofills
...
CardJs.prototype.expiryMonth = function () {
  if (!this.EXPIRY_USE_DROPDOWNS && this.expiryMonthYearInput != null) {
    var val = this.expiryMonthYearInput.val();
    //return val;
    return val.length >= 2 ? parseInt(val.substr(0, 2), 10) : null;
